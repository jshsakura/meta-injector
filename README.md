...
- ✅ **통합 GCT 패치 시스템**:
  - 200개 이상의 게임별 클래식 컨트롤러 패치
  - 게임패드 강제 적용 자동 감지 (호환성 DB 기반)
  - Galaxy 1/2 게임패드 패치 (AllStars/Nvidia 프로필)
  - 사용자 정의 Generic 패치 지원
...
### 이미지 & 메타데이터
- 🖼️ **스마트 이미지 캐싱**:
  - 뱃지 타입별 자동 캐싱 (icon.png, icon_gct.png, icon_allstars.png)
  - 사용자 선택 이미지 보존 (재빌드 시 유지)
  - 영구 캐시로 후속 빌드 속도 향상
- 🌐 **GameTDB 통합**:
  - 게임 제목 및 커버 아트 자동 가져오기
  - 편집 다이얼로그에서 이미지 재다운로드 기능
  - 다운로드 시 제목 자동 업데이트 (캐시 및 DB 갱신)
- 🔍 **호환성 데이터베이스**:
  - 게임패드 호환성 정보 내장
  - GCT 패치 자동 감지 및 표시
  - force_cc 플래그 관리
- ✏️ **간편한 편집**:
  - GUI를 통한 게임 메타데이터, 제목, 이미지 편집
  - ISO 트림 비활성화 옵션 (세이브 파일 문제 해결)
  - WBFS 파일 자동 감지 및 제한 표시
...
1. **⚙ 설정** 버튼 클릭
2. 암호화 키 입력:
   - **Wii U Common Key** (필수) - 빨간색/녹색 강조 표시
   - **Rhythm Heaven Fever Title Key** (필수) - 빨간색/녹색 강조 표시
   - **Xenoblade Chronicles Title Key** (선택)
   - **Super Mario Galaxy 2 Title Key** (선택)
   - **Ancast Key (Wii U Starbuck Key)** (C2W에 필수) - 노란색 강조 표시
     - CPU 클럭 해제 패치(729MHz → 1.215GHz)를 위해 필요
...
3. (선택사항) **편집**을 클릭하여 사용자 지정:
   - 게임 제목, 이미지, 베이스 ROM 선택
   - **이미지 자동 다운로드** 버튼으로 GameTDB에서 최신 이미지/제목 가져오기
   - **ISO 트림 비활성화** 체크박스 (세이브 파일 문제 해결용)
     - 일부 게임은 원본 디스크 크기(약 4~8GB) 필요
     - WBFS 파일은 이미 트림된 포맷으로 복원 불가
   - **✅ Trucha 버그 패치** (기본: 활성화)
     - 서명 검증 우회, 수정된 게임에 필요
   - **✅ C2W CPU 클럭 해제 패치** (기본: 활성화, Ancast 키 필요)
     - CPU를 729MHz에서 1.215GHz로 상향
4. 드롭다운에서 게임패드 프로필 선택
   - GCT 패치가 있는 게임은 자동으로 표시 ("GCT 패치 가능" 라벨)
5. **빌드 시작** 클릭
...
5. **펌웨어 패치 적용**
   - **Trucha 버그 패치** (기본: 활성화)
     ...
     - 오류 22000 해결
     - 수정된 게임 실행에 필수
   - **C2W CPU 클럭 해제** (Ancast 키 제공 시)
     - c2w.img 패치
     - CPU 클럭: 729MHz → 1.215GHz
     - 고사양 게임 성능 향상
